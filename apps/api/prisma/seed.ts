...
async function main() {
  ...
  for (let i = 0; i < memberData.length; i++) {
    const m = await prisma.member.create({
      data: { ...memberData[i], memberId: `IFFE-${String(i + 1).padStart(3, "0")}` },
    });
    members.push(m);
  }
  ...
  // ===== ACCOUNTS (20) =====
  const balances = [
    2500000, 1800000, 3200000, 500000, 4100000, 750000, 0, 1200000, 5600000, 980000, 2300000, 150000, 3800000, 620000,
    0, 1500000, 8200000, 4500000, 950000, 2100000,
  ];
  const types = [
    "savings",
    "fixed_deposit",
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const acc = await prisma.account.create({
      data: {
        accountNo: `${prefix}-${String(i + 1).padStart(4, "0")}`,
        memberId: members[mIdx].id,
        type: types[i],
        balance: balances[i],
        interestRate: rate,
        status: statuses[i],
        lastActivity: daysAgo(rand(0, 30)),
      },
    });
    accounts.push(acc);
  }
  ...
  const expData = [
    ...
    { desc: "System Maintenance", cat: "Technical Issues", amount: 500000, status: "approved" },
    ...
    { desc: "Software Licenses", cat: "Technical Issues", amount: 750000, status: "pending" },
    ...
    { desc: "Insurance Premium", cat: "Operations", amount: 1500000, status: "approved" },
  ...
  for (let i = 0; i < expData.length; i++) {
    await prisma.expense.create({
      data: {
        description: expData[i].desc,
        category: expData[i].cat,
        amount: expData[i].amount,
        status: expData[i].status,
        date: daysAgo(expData.length * 3 - i * 3),
        approvedBy: expData[i].status === "approved" ? users[0].id : undefined,
      },
    });
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    await prisma.depositRequest.create({
      data: {
        memberId: members[mIdx].id,
        accountId: accounts[mIdx].id,
        amount: rand(100000, 2000000),
        method: ["cash", "mobile_money", "bank_transfer"][i % 3],
        description: `Deposit request #${i + 1}`,
        status,
        processedBy: status !== "pending" ? users[0].id : undefined,
  ...
  const w1 = await prisma.welfareProgram.create({
    data: {
      name: "Medical Emergency Fund",
      description: "Emergency medical support for members and families",
  ...
  const w2 = await prisma.welfareProgram.create({
    data: {
      name: "Education Scholarship",
      description: "Supporting member children's education through bursaries",
  ...
  const w3 = await prisma.welfareProgram.create({
    data: {
      name: "Bereavement Support",
      description: "Financial support during loss of a family member",
  ...
  const w4 = await prisma.welfareProgram.create({
    data: {
      name: "Housing Support Fund",
      description: "Help members with housing construction and improvements",
  ...
  for (let i = 0; i < 25; i++) {
    ...
    await prisma.pledge.create({
      data: {
        ...
        status: pledgeStatuses[i % pledgeStatuses.length],
      },
    });
  }
