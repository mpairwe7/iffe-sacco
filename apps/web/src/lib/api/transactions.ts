...
export const transactionsApi = {
  ...
  approveTransaction: (id: string) =>
    apiClient.patch<Transaction>(`/transactions/${id}/approve`),
  rejectTransaction: (id: string) =>
    apiClient.patch<Transaction>(`/transactions/${id}/reject`),
  getTransactionStats: () =>
    apiClient.get<Record<string, unknown>>("/transactions/stats"),
};
