...
const allPages = [
  { label: "Admin Dashboard", href: "/dashboard", icon: LayoutDashboard, group: "Pages", roles: ["admin", "staff"] as Role[] },
  { label: "Chairman Dashboard", href: "/chairman", icon: LayoutDashboard, group: "Pages", roles: ["chairman"] as Role[] },
...
export function CommandPalette() {
  ...
  const [searchResults, setSearchResults] = useState<Array<{ id: string; firstName: string; lastName: string; memberId: string }>>([]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setSearching(true);
      try {
        const data = await apiClient.get<{ data: Array<{ id: string; firstName: string; lastName: string; memberId: string }> }>("/members", { search: query, limit: 5 });
          ...
          <Command.List className="max-h-80 overflow-y-auto p-2">
              ...
              <Command.Group heading="Members" className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-2 pt-3 pb-1">
                {searchResults.map((m) => (
                  <Command.Item
                    ...
                    onSelect={() => { router.push(`/admin/members/${m.id}`); closePalette(); }}
              ...
              <Command.Group key={group} heading={group} className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-2 pt-3 pb-1">
                {pages.filter((p) => p.group === group).map((page) => (
                  <Command.Item
                    key={page.href}
                    value={page.label}
                    onSelect={() => { router.push(page.href); closePalette(); }}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer data-[selected=true]:bg-primary/10 data-[selected=true]:text-primary"
                  >
            ...
            <Command.Group heading="Actions" className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-2 pt-3 pb-1">
              <Command.Item
                ...
                onSelect={() => { setTheme(theme === "dark" ? "light" : "dark"); closePalette(); }}
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer data-[selected=true]:bg-primary/10 data-[selected=true]:text-primary"
              >
