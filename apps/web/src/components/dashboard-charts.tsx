...
interface LoanPoint { month: string; label: string; disbursed: number; repaid: number }
      ...
      {monthlyData.length === 0 ? (
        ...
        <figure role="figure" aria-label="Monthly deposits vs withdrawals chart">
          <div className="sr-only" role="img" aria-label="Deposits and Withdrawals area chart">
            <ul>{monthlyData.map(d => <li key={d.month}>{d.label}: Deposits USh {d.deposits.toLocaleString()}, Withdrawals USh {d.withdrawals.toLocaleString()}</li>)}</ul>
          </div>
      ...
      {breakdownData.length === 0 ? (
        ...
        <figure role="figure" aria-label="Expense distribution by category chart">
          <div className="sr-only" role="img" aria-label="Expense pie chart">
            <ul>{breakdownData.map(d => <li key={d.name}>{d.name}: {d.value}%</li>)}</ul>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
      ...
      {trendsData.length === 0 ? (
        ...
        <figure role="figure" aria-label="Loan disbursements vs repayments chart">
          <div className="sr-only" role="img" aria-label="Loan bar chart">
            <ul>{trendsData.map(d => <li key={d.month}>{d.label}: Disbursed USh {d.disbursed.toLocaleString()}, Repaid USh {d.repaid.toLocaleString()}</li>)}</ul>
          </div>
          <ResponsiveContainer width="100%" height={280}>
