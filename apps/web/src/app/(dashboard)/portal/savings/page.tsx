    ...
    <div className="space-y-6">
      ...
      {accountsLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <StatCard
            title="Interest Rate"
            value={accounts[0] ? `${accounts[0].interestRate}% p.a.` : "N/A"}
            icon={TrendingUp}
            color="success"
          />
          ...
          {accounts.map((acc) => (
            ...
            >
              ...
              <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                Interest Rate: {acc.interestRate}% p.a.
              </p>
              <div className="flex gap-3 mt-4">
                <Link
                  href="/portal/deposits"
                  className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary-dark"
                >
                  <ArrowDownToLine className="w-4 h-4" /> Deposit
                </Link>
                <Link
                  href="/portal/withdrawals"
                  className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-text border border-border rounded-lg hover:bg-surface-alt"
                >
              ...
              recentActivity.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-center gap-4 px-6 py-4">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${isOutflow ? "bg-warning/15" : "bg-success/15"}`}
                    >
                      {isOutflow ? (
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-text-muted">
                        {item.method} &middot; {formatDate(item.createdAt)}
                      </p>
                    </div>
