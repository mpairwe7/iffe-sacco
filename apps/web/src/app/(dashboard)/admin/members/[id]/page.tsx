...
import type { MemberDashboard } from "@iffe/shared";
...
function formatSupportStatus(status: "received" | "not_received") {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          hint="Captured from member account details"
        />
        <StatCard
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/50">
                    <th className="text-left text-xs font-semibold text-text-muted uppercase tracking-wider px-4 py-2">Account No.</th>
                    <th className="text-left text-xs font-semibold text-text-muted uppercase tracking-wider px-4 py-2">Type</th>
                    <th className="text-left text-xs font-semibold text-text-muted uppercase tracking-wider px-4 py-2">Status</th>
                    <th className="text-right text-xs font-semibold text-text-muted uppercase tracking-wider px-4 py-2">Balance</th>
                  </tr>
                </thead>
                <tbody>
              ...
              recentTransactions.map((transaction) => {
                ...
                return (
                  <div key={transaction.id} className="py-3 flex items-start justify-between gap-4">
                    ...
                    <p className={`text-sm font-semibold ${isOutflow ? "text-warning" : "text-success"}`}>
          ...
          <div className="space-y-3">
            {pledges.length === 0 ? (
              ...
              pledges.slice(0, 5).map((pledge) => (
                <div key={pledge.id} className="flex items-center justify-between gap-4 rounded-xl border border-border/60 px-4 py-3">
                  <div>
