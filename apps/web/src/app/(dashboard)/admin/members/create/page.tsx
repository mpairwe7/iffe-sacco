...
import { ArrowLeft, Save, UserPlus, Loader2, KeyRound, Copy, Check } from "lucide-react";
...
export default function CreateMemberPage() {
  ...
  useEffect(() => {
    if (userRole && userRole !== "admin" && userRole !== "staff") {
      toast.error("Only staff or administrators can add members directly");
  ...
  async function onSubmit(data: MemberCreateInput) {
    try {
      const result = await createMember.mutateAsync(data as Parameters<typeof createMember.mutateAsync>[0]);
      toast.success("Member created");
      // Surface the one-time temporary password for staff to hand over; the
      // member is forced to set their own password on first login.
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md glass-card rounded-xl p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-lg font-semibold text-text">Member created</h2>
            </div>
