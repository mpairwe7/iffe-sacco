...
auditLogs.get("/", zValidator("query", paginationSchema), async (c) => {
  ...
  const [data, total] = await Promise.all([
    prisma.auditLog.findMany({ where, skip, take: limit, orderBy: { createdAt: sortOrder }, include: { user: { select: { name: true, email: true } } } }),
...
auditLogs.get("/:id", async (c) => {
  ...
  const log = await prisma.auditLog.findUnique({ where: { id }, include: { user: { select: { name: true, email: true } } } });
